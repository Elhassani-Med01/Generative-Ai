// src/components/workflow-panels/SketchToImage.jsx
...
const SketchToImage = ({ 
  ...
  return (
    <div className="space-y-4">
      <PromptChatbot
        context={PROMPT_CONTEXTS["sketch-to-image"].prompt}
        onPrompt={p => { setPrompt(p); setLastPrompt(p); }}
      />

      {/* Sketch upload */}
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-2">Esquisse Source</label>
        <div
        ...
        >
          {uploadedImage ? (
            <img src={uploadedImage} alt="Uploaded Sketch" className="w-full h-full object-cover rounded-lg" />
          ) : (
            <div className="text-gray-400 text-center">
              <Upload className="mx-auto mb-2" />
              <span>Téléchargez votre esquisse</span>
            </div>
          )}
        </div>
        <input
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Décrivez l'image finale souhaitée..."
          rows="3"
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Ce que vous voulez éviter dans l'image finale..."
          rows="2"
          className="w-full bg-gray-700 text-white px-3 py-2 rounded-lg border border-gray-600 focus:border-purple-500 focus:outline-none resize-none"
        />
      </div>
      {showFeedback && (
        <FeedbackChatbot
          ...
          context={PROMPT_CONTEXTS["sketch-to-image"].feedback}
